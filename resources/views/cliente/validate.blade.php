@extends('templates/main', ['titulo'=>"VALIDAÇÃO - CADASTRO DE CLIENTE"])
            ...
            <tbody>
                @foreach ($data as $item)
                    <form action="{{ route('validate.finish', $item->id) }}" method="POST" id="form_{{$item->id}}">
                        ...
                        <tr>
                            <td>{{$item->nome}}</td>
                            <td class="d-none d-md-table-cell">{{$item->projeto}}</td>
                            <td>
                                <a nohref style="projeto:pointer" onclick="showConfirmModal('{{$item->id}}', '{{$item->nome}}', false)">
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function showConfirmModal(id, nome, response) {
            ...
            $('#nome_cliente').removeClass()

            if(response) {
                $(input_status).val(1)
                $('#title_modal').addClass("modal-title text-success")
                $('#nome_cliente').addClass("fw-bold text-success")
                ...
                $('#nome_cliente').addClass("fw-bold text-danger")
